...
public class MemoryTable extends AbstractTable implements ScannableTable {
    ...
    public RelDataType getRowType(RelDataTypeFactory typeFactory) {
        if(dataType == null) {
            ...
            for (MemoryData.Column column: this.sourceTable.columnList) {
                RelDataType sqlType = typeFactory.createJavaType(MemoryData.gClassTypeMap.get(column.type));
    ...
    public Enumerable<Object[]> scan(DataContext root) {
        final List<String> types = new ArrayList<>(sourceTable.columnList.size());
        for (MemoryData.Column column: sourceTable.columnList) {
            types.add(column.name);
        }
